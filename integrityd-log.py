...
#                    print pyline
                ...
                try:
                    ...
                    self._special('Bad line in "{}" with "{}" ignored: "{}"'.format(os.path.join ( path, item ), e.args[0], line))
            # all done
        ...
        for path in self.dirstate:
            ...
            for item in self.rules[path]:
                if not os.path.isfile(os.path.join( path, item)):
                    ...
                    if not startup: self._special('Removed rule file: {} "{}" "{}"'.format(os.path.join(path,item),path,item))    # inform
    ...
    def _readlog ( self, logfile, lastinode, lastposition ):
        ...
        try:
            ...
            if logfile not in self.lasterror or self.lasterror[logfile] + config['common']['errornag'] > time.time():
                self._special("Failed opening logfile {} with: {}".format(logfile, e))
